...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all bullex-ai.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*bullex-ai\.sh$/)) {
        return true;
      }

      // Allow all bullex-ai.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*bullex-ai\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@bullex-ai.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@bullex-ai.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
